...
const PhishingWarningBanner: React.FC<React.PropsWithChildren> = () => {
  ...
  const warningTextAsParts = useMemo(() => {
    const warningText = t(
      "please make sure you're visiting https://orkswap-frontend.vercel.app - check the URL carefully.",
    )
    return warningText.split(/(https:\/\/orkswap-frontend.vercel.app)/g)
  }, [t])
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://orkswap-frontend.vercel.app'}
          color={text === 'https://orkswap-frontend.vercel.app' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
